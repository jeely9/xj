...
function Indexsname(seName,province,city,district,year){
    $.ajax({
        type:"get",
        url:"http://"+host+"/financial/getFinancialQueryField?typeName=%E4%BF%9D%E9%99%A9",
        async:true,
        success: function(data){
            var datas = data.val;
            // console.log(datas);
...
function Tabledata(fieldNames,seName,province,city,district,year,sortColumn,sortSC){
    ...
    $.ajax({
        type:"get",
        url:"http://"+host+"/financial/fiList?pageNo=1&typeName=%E4%BF%9D%E9%99%A9",
        data:Data,
        async:true,
        success: function(data){
            // console.log(data);
...
function TabledataList(datas){
    ...
    $("#bankBoxTable tr td:nth-child(3)").click(function(){
        ...
        //给下一页传参
        window.location.href="safespecificData.html?seId="+seId;
    })
...
function tablePage(datas,fieldNames,seName,province,city,district,year,sortColumn,sortSC){
    ...
    function pageCallback(page_index,jq) {  //page_index表示当前点击的那个分页的页数索引值默认为0，后一个参数表示装载容器。
        ...
        $.ajax({
            type:"get",
            url:"http://"+host+"/financial/fiList?typeName=%E4%BF%9D%E9%99%A9",
            data:Datas,
            async:true,
            success:function(data){
                var datas = data.val;
