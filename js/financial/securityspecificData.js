var width = (document.body.clientWidth)*98/100*32/100+'px';
...
var resizeCss = function() {
    ...
    $("#bussAssurepie").css("width", width1).css("height", height2);
};
resizeCss();
...
var option = option1 = option1 = option3 =  option8 =null;
option = option1 = option2 = option3  =option8 ={
    title : {
        //text: '出资占比',
        //subtext: '纯属虚构',
        x:'center'
...
function businessEpie(bussEpie1data){
...
function businessEchars(list1){
    var bussEpie1name = [];
    var bussEpie1value = [];
    var bussEpie1data = [];
    for (var i =0;i<list1.length;i++){
        bussEpie1name.push(list1[i].name);
        bussEpie1value.push((list1[i].value[1]).replace(/%/g, ""));
        bussEpie1data.push({name:list1[i].name,value:(list1[i].value[1]).replace(/%/g, "")});
    }
    businessEpie(bussEpie1data);
...
function businessYear(seId,year){
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            $("#sebusstitle").html(datas[0].title);
            businessTable(datas);
...
function businessTable(datas){
    ...
    for (var i =0;i<list1.length;i++){
        ...
        for (var j=0;j<list1[i].value.length;j++){
            if(list1[i].value.length<2){
                tableconcent1 += '<td>'+list1[i].value[j]+'</td>';
                for(var k=0;k<(2-list1[i].value.length);k++){
                    tableconcent1 += '<td></td>';
                }
            }else{
                tableconcent1 += '<td>'+list1[i].value[j]+'</td>';
    ...
    $("#businessTable").html(tableconcent1);
    businessEchars(list1);
}
...
function newTableData(newtab){
    var box = "";
    for(var i=0;i<newtab.length;i++){
        box += '<tr><td>'+(i+1)+'</td><td>'+newtab[i].shareholders+'</td>';
        box += '<td>'+newtab[i].contributionAmount+'</td><td>'+newtab[i].ratio+'</td><td>'+newtab[i].sbTypeName+'</td>';
    }
    box += '</tr>';
    $("#trtemplate").html(box);
