...
function move(imId){
    ...
    $.ajax({
        ...
        // url:"http://192.168.3.4:8081/industry/getOperationInformation?periods=5",
        // url:"../../js/industry/all.json",
        data:{"imId":imId},
        async:true,
        success:function(data){
            var datas = data.val;
...
function fiveTable(datas){
    ...
    // var title = datas.year;
    // var list = datas.list;
    // var TitleThirdContent = '';
    // var TableThirdContent = '';
    // TitleThirdContent += '<tr>';
    // for (var i = 0;i<title.time.length;i++){
    //     TitleThirdContent += '<th>'+title.time[i]+'</th>';
    // }
    // TitleThirdContent += '</tr>';
    // $("#allFiveHeader").html(TitleThirdContent);
...
function fiveAllPhoto(datas){
    if(datas.length>=9){
        ...
        var values31 = [];
        var finals3 = [];
        name3.push(datas[5][0],datas[6][0]);
        for (var i=1;i<datas[0].length;i++){
            years.push(datas[0][i]);
        }
        for (var m=1;m<datas[5].length;m++){
            values3.push(datas[5][m]);
        }
        for (var n=1;n<datas[6].length;n++){
            values31.push(datas[6][n]);
        }
        finals3.push({name:datas[5][0],type:'bar',data:values3},{name:datas[6][0],type:'bar',data:values31});
        name3FiveEchars(years,name3,finals3,Three);
    }
    var years = [];
    var name1 = [];
